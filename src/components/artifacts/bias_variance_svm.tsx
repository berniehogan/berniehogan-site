...
const BiasVarianceSVM = () => {
  ...
  const computeDecisionBoundary = (data, regularization) => {
    const width = 400;
    const height = 400;
    const grid = [];
    const step = 8;
  ...
  const calculateMetrics = (data, decisionBoundary) => {
  ...
  useEffect(() => {
    const boundary = computeDecisionBoundary(trainingData, C[0]);
  ...
  const drawCanvas = () => {
    ...
    decisionBoundary.forEach(point => {
      ctx.fillStyle = classes[point.class].color + '15';
    ...
    trainingData.forEach(point => {
      ctx.fillStyle = classes[point.class].color;
      ctx.beginPath();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-lg p-4 shadow-lg">
          ...
          <div className="mt-6 pt-4 border-t">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Regularization Parameter (C): {C[0].toFixed(2)}
            </label>
            <input
              type="range"
              value={C[0]}
            ...
            <div className="flex justify-between text-xs text-gray-500 mt-1">
              ...
              <span>Low Regularization<br/>(Complex boundaries)</span>
            </div>
          </div>
        </div>
